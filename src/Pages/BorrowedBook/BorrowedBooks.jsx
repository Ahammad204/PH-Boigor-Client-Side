  ...
  const handleDelete = (_id, bookId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://ph-boigor-server-side.onrender.com/borrowed/${_id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
            ...
            if (data.deletedCount > 0) {
              ...
              const fetchBooKIdData = async () => {
                const response = await fetch(
                  `https://ph-boigor-server-side.onrender.com/book/${bookId}`
                );
                ...
                fetch(`https://ph-boigor-server-side.onrender.com/book/${bookId}`, {
                  method: "PUT",
                  headers: {
      ...
      {bookings.length > 0 ? (
        <div>
          <h1 className="text-center font-bold text-6xl mb-5 mt-5">
            {" "}
            Your Borrowed <span className="text-[#E59285]"> Books</span>
          </h1>
          <hr />
              ...
              <div className="card bg-base-100 shadow-xl" key={prod._id}>
                ...
                <div className="card-body">
                  ...
                  <p className="text-base font-semibold mt-4">
                    Borrowed Date: {prod.today}
                  </p>
                  <p className="text-base font-semibold ">
                    Return Date: {prod.returnDate}
                  </p>

                  <div className="card-actions justify-end">
                    <button
                      onClick={() => handleDelete(prod._id, prod.bookId)}
                      className="btn text-white border-none  bg-[#E59285] hover:bg-[#E59285] "
                    >
                      Return
                    </button>
                  </div>
          ...
          {!isLoading ? (
            ...
            <div className="flex justify-center items-center mt-20">
              <div className="hero w-2/3 h-2/3">
                <div className=""></div>
                <div className="hero-content text-center ">
                  <div className="mw-ax-md">
                    <h1 className="mb-5 text-5xl font-bold">Hey Book Lover</h1>
                    <p className="mb-5 text-xl ">
                      There is noting to show <br /> Borrow Some Book For
                      Explore
                    </p>
                  </div>
                </div>
